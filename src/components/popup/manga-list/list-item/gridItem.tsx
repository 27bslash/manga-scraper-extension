...
export const GridItem = (props: gridItemProps) => {
    ...
    chrome.storage.local.get("manga-list", (res) => {
      res["manga-list"].forEach((element: Manga) => {
        if (element.title === props.title) {
          element.read = true;
          element.chapter = element.sources[props.currentSource].latest;
          element.sources[props.currentSource]["chapter"] =
            element.sources[props.currentSource]["latest"];
          element.sources[props.currentSource]["url"] =
            element.sources[props.currentSource]["latest_link"];

          console.log("update read for: ", element);
        }
      });
  ...
  return (
    <>
      ...
      <Grid item xs={3} sm={3} md={3} alignItems={"end"}>
        <OptionsMenu
